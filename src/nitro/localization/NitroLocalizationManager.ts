...
export class NitroLocalizationManager extends NitroManager implements INitroLocalizationManager
{
    ...
    constructor(communication: INitroCommunicationManager)
    {
        ...
        this._pendingUrls = [];
        this._allowedLangs = ["es", "en", "fr", "de", "tr", "pt"];
        this._externalTextsLang = "de";
    }

    protected onInit(): void {
        this._communication.registerMessageEvent(new BadgePointLimitsEvent(this.onBadgePointLimitsEvent.bind(this)));
      
        if (!localStorage.getItem("nitro-lang")) {
          const userLang = navigator.language;
          const language_parts = userLang.split("-");
          const language = language_parts[0];
      
          if (this._allowedLangs.includes(language)) {
            this._externalTextsLang = language;
          }
      
          localStorage.setItem("nitro-lang", this._externalTextsLang);
        } else {
          this._externalTextsLang = localStorage.getItem("nitro-lang");
        }
        ...
        for (let i = 0; i < urls.length; i++) {
          urls[i] = Nitro.instance.core.configuration.interpolate(urls[i]);
        }
